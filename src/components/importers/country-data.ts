export interface CountryDef {
  code: string;
  isoA2: string;
  isoA3: string;
  name: string;
  englishName: string;
  continent: string;
  dbAliases: string[];
}

export const CONTINENT_ORDER = ['Europe', 'Amériques', 'Asie-Pacifique', 'Moyen-Orient', 'Afrique', 'Océanie'] as const;

export const COUNTRIES: CountryDef[] = [
  {
    "code": "ZA",
    "isoA2": "ZA",
    "isoA3": "ZAF",
    "name": "Afrique du Sud",
    "englishName": "South Africa",
    "continent": "Afrique",
    "dbAliases": [
      "South Africa"
    ]
  },
  {
    "code": "AO",
    "isoA2": "AO",
    "isoA3": "AGO",
    "name": "Angola",
    "englishName": "Angola",
    "continent": "Afrique",
    "dbAliases": [
      "Angola"
    ]
  },
  {
    "code": "BW",
    "isoA2": "BW",
    "isoA3": "BWA",
    "name": "Botswana",
    "englishName": "Botswana",
    "continent": "Afrique",
    "dbAliases": [
      "Botswana"
    ]
  },
  {
    "code": "CM",
    "isoA2": "CM",
    "isoA3": "CMR",
    "name": "Cameroun",
    "englishName": "Cameroon",
    "continent": "Afrique",
    "dbAliases": [
      "Cameroon"
    ]
  },
  {
    "code": "CV",
    "isoA2": "CV",
    "isoA3": "CPV",
    "name": "Cap-Vert",
    "englishName": "Cape Verde",
    "continent": "Afrique",
    "dbAliases": [
      "Cape Verde"
    ]
  },
  {
    "code": "CI",
    "isoA2": "CI",
    "isoA3": "CIV",
    "name": "Côte d’Ivoire",
    "englishName": "Cote D`ivoire",
    "continent": "Afrique",
    "dbAliases": [
      "Cote D`ivoire"
    ]
  },
  {
    "code": "DJ",
    "isoA2": "DJ",
    "isoA3": "DJI",
    "name": "Djibouti",
    "englishName": "Djibouti",
    "continent": "Afrique",
    "dbAliases": [
      "Djibouti"
    ]
  },
  {
    "code": "GA",
    "isoA2": "GA",
    "isoA3": "GAB",
    "name": "Gabon",
    "englishName": "Gabon",
    "continent": "Afrique",
    "dbAliases": [
      "Gabon"
    ]
  },
  {
    "code": "GH",
    "isoA2": "GH",
    "isoA3": "GHA",
    "name": "Ghana",
    "englishName": "Ghana",
    "continent": "Afrique",
    "dbAliases": [
      "Ghana"
    ]
  },
  {
    "code": "KE",
    "isoA2": "KE",
    "isoA3": "KEN",
    "name": "Kenya",
    "englishName": "Kenya",
    "continent": "Afrique",
    "dbAliases": [
      "Kenya"
    ]
  },
  {
    "code": "MG",
    "isoA2": "MG",
    "isoA3": "MDG",
    "name": "Madagascar",
    "englishName": "Madagascar",
    "continent": "Afrique",
    "dbAliases": [
      "Madagascar"
    ]
  },
  {
    "code": "MA",
    "isoA2": "MA",
    "isoA3": "MAR",
    "name": "Maroc",
    "englishName": "Morocco",
    "continent": "Afrique",
    "dbAliases": [
      "Morocco"
    ]
  },
  {
    "code": "MZ",
    "isoA2": "MZ",
    "isoA3": "MOZ",
    "name": "Mozambique",
    "englishName": "Mozambique",
    "continent": "Afrique",
    "dbAliases": [
      "Mozambique"
    ]
  },
  {
    "code": "NA",
    "isoA2": "NA",
    "isoA3": "NAM",
    "name": "Namibie",
    "englishName": "Namibia",
    "continent": "Afrique",
    "dbAliases": [
      "Namibia"
    ]
  },
  {
    "code": "NG",
    "isoA2": "NG",
    "isoA3": "NGA",
    "name": "Nigéria",
    "englishName": "Nigeria",
    "continent": "Afrique",
    "dbAliases": [
      "Nigeria"
    ]
  },
  {
    "code": "UG",
    "isoA2": "UG",
    "isoA3": "UGA",
    "name": "Ouganda",
    "englishName": "Uganda",
    "continent": "Afrique",
    "dbAliases": [
      "Uganda"
    ]
  },
  {
    "code": "RW",
    "isoA2": "RW",
    "isoA3": "RWA",
    "name": "Rwanda",
    "englishName": "Rwanda",
    "continent": "Afrique",
    "dbAliases": [
      "Rwanda"
    ]
  },
  {
    "code": "SH",
    "isoA2": "SH",
    "isoA3": "SHN",
    "name": "Sainte-Hélène",
    "englishName": "St. Helena",
    "continent": "Afrique",
    "dbAliases": [
      "St. Helena"
    ]
  },
  {
    "code": "SC",
    "isoA2": "SC",
    "isoA3": "SYC",
    "name": "Seychelles",
    "englishName": "Seychelles",
    "continent": "Afrique",
    "dbAliases": [
      "Seychelles"
    ]
  },
  {
    "code": "SN",
    "isoA2": "SN",
    "isoA3": "SEN",
    "name": "Sénégal",
    "englishName": "Senegal",
    "continent": "Afrique",
    "dbAliases": [
      "Senegal"
    ]
  },
  {
    "code": "TZ",
    "isoA2": "TZ",
    "isoA3": "TZA",
    "name": "Tanzanie",
    "englishName": "Tanzania",
    "continent": "Afrique",
    "dbAliases": [
      "Tanzania"
    ]
  },
  {
    "code": "ZM",
    "isoA2": "ZM",
    "isoA3": "ZMB",
    "name": "Zambie",
    "englishName": "Zambia",
    "continent": "Afrique",
    "dbAliases": [
      "Zambia"
    ]
  },
  {
    "code": "ZW",
    "isoA2": "ZW",
    "isoA3": "ZWE",
    "name": "Zimbabwe",
    "englishName": "Zimbabwe",
    "continent": "Afrique",
    "dbAliases": [
      "Zimbabwe"
    ]
  },
  {
    "code": "EG",
    "isoA2": "EG",
    "isoA3": "EGY",
    "name": "Égypte",
    "englishName": "Egypt",
    "continent": "Afrique",
    "dbAliases": [
      "Egypt"
    ]
  },
  {
    "code": "ET",
    "isoA2": "ET",
    "isoA3": "ETH",
    "name": "Éthiopie",
    "englishName": "Ethiopia",
    "continent": "Afrique",
    "dbAliases": [
      "Ethiopia"
    ]
  },
  {
    "code": "MU",
    "isoA2": "MU",
    "isoA3": "MUS",
    "name": "Île Maurice",
    "englishName": "Mauritius",
    "continent": "Afrique",
    "dbAliases": [
      "Mauritius"
    ]
  },
  {
    "code": "AG",
    "isoA2": "AG",
    "isoA3": "ATG",
    "name": "Antigua-et-Barbuda",
    "englishName": "Antigua and Barbuda",
    "continent": "Amériques",
    "dbAliases": [
      "Antigua and Barbuda"
    ]
  },
  {
    "code": "AR",
    "isoA2": "AR",
    "isoA3": "ARG",
    "name": "Argentine",
    "englishName": "Argentina",
    "continent": "Amériques",
    "dbAliases": [
      "Argentina"
    ]
  },
  {
    "code": "AW",
    "isoA2": "AW",
    "isoA3": "ABW",
    "name": "Aruba",
    "englishName": "Aruba",
    "continent": "Amériques",
    "dbAliases": [
      "Aruba"
    ]
  },
  {
    "code": "BS",
    "isoA2": "BS",
    "isoA3": "BHS",
    "name": "Bahamas",
    "englishName": "Bahamas",
    "continent": "Amériques",
    "dbAliases": [
      "Bahamas"
    ]
  },
  {
    "code": "BB",
    "isoA2": "BB",
    "isoA3": "BRB",
    "name": "Barbade",
    "englishName": "Barbados",
    "continent": "Amériques",
    "dbAliases": [
      "Barbados"
    ]
  },
  {
    "code": "BM",
    "isoA2": "BM",
    "isoA3": "BMU",
    "name": "Bermudes",
    "englishName": "Bermuda",
    "continent": "Amériques",
    "dbAliases": [
      "Bermuda"
    ]
  },
  {
    "code": "BO",
    "isoA2": "BO",
    "isoA3": "BOL",
    "name": "Bolivie",
    "englishName": "Bolivia",
    "continent": "Amériques",
    "dbAliases": [
      "Bolivia"
    ]
  },
  {
    "code": "BQ",
    "isoA2": "BQ",
    "isoA3": "BES",
    "name": "Bonaire",
    "englishName": "Bonaire",
    "continent": "Amériques",
    "dbAliases": [
      "Bonaire"
    ]
  },
  {
    "code": "BR",
    "isoA2": "BR",
    "isoA3": "BRA",
    "name": "Brésil",
    "englishName": "Brazil",
    "continent": "Amériques",
    "dbAliases": [
      "Brazil"
    ]
  },
  {
    "code": "CA",
    "isoA2": "CA",
    "isoA3": "CAN",
    "name": "Canada",
    "englishName": "Canada",
    "continent": "Amériques",
    "dbAliases": [
      "Canada"
    ]
  },
  {
    "code": "CL",
    "isoA2": "CL",
    "isoA3": "CHL",
    "name": "Chili",
    "englishName": "Chile",
    "continent": "Amériques",
    "dbAliases": [
      "Chile"
    ]
  },
  {
    "code": "CO",
    "isoA2": "CO",
    "isoA3": "COL",
    "name": "Colombie",
    "englishName": "Colombia",
    "continent": "Amériques",
    "dbAliases": [
      "Colombia"
    ]
  },
  {
    "code": "CR",
    "isoA2": "CR",
    "isoA3": "CRI",
    "name": "Costa Rica",
    "englishName": "Costa Rica",
    "continent": "Amériques",
    "dbAliases": [
      "Costa Rica"
    ]
  },
  {
    "code": "CU",
    "isoA2": "CU",
    "isoA3": "CUB",
    "name": "Cuba",
    "englishName": "Cuba",
    "continent": "Amériques",
    "dbAliases": [
      "Cuba"
    ]
  },
  {
    "code": "CW",
    "isoA2": "CW",
    "isoA3": "CUW",
    "name": "Curaçao",
    "englishName": "Curacao",
    "continent": "Amériques",
    "dbAliases": [
      "Curacao"
    ]
  },
  {
    "code": "GT",
    "isoA2": "GT",
    "isoA3": "GTM",
    "name": "Guatemala",
    "englishName": "Guatemala",
    "continent": "Amériques",
    "dbAliases": [
      "Guatemala"
    ]
  },
  {
    "code": "HT",
    "isoA2": "HT",
    "isoA3": "HTI",
    "name": "Haïti",
    "englishName": "Haiti",
    "continent": "Amériques",
    "dbAliases": [
      "Haiti"
    ]
  },
  {
    "code": "HN",
    "isoA2": "HN",
    "isoA3": "HND",
    "name": "Honduras",
    "englishName": "Honduras",
    "continent": "Amériques",
    "dbAliases": [
      "Honduras"
    ]
  },
  {
    "code": "JM",
    "isoA2": "JM",
    "isoA3": "JAM",
    "name": "Jamaïque",
    "englishName": "Jamaica",
    "continent": "Amériques",
    "dbAliases": [
      "Jamaica"
    ]
  },
  {
    "code": "MQ",
    "isoA2": "MQ",
    "isoA3": "MTQ",
    "name": "Martinique",
    "englishName": "Martinique",
    "continent": "Amériques",
    "dbAliases": [
      "Martinique"
    ]
  },
  {
    "code": "MX",
    "isoA2": "MX",
    "isoA3": "MEX",
    "name": "Mexique",
    "englishName": "Mexico",
    "continent": "Amériques",
    "dbAliases": [
      "Mexico"
    ]
  },
  {
    "code": "NI",
    "isoA2": "NI",
    "isoA3": "NIC",
    "name": "Nicaragua",
    "englishName": "Nicaragua",
    "continent": "Amériques",
    "dbAliases": [
      "Nicaragua"
    ]
  },
  {
    "code": "PA",
    "isoA2": "PA",
    "isoA3": "PAN",
    "name": "Panama",
    "englishName": "Panama",
    "continent": "Amériques",
    "dbAliases": [
      "Panama"
    ]
  },
  {
    "code": "PY",
    "isoA2": "PY",
    "isoA3": "PRY",
    "name": "Paraguay",
    "englishName": "Paraguay",
    "continent": "Amériques",
    "dbAliases": [
      "Paraguay"
    ]
  },
  {
    "code": "PR",
    "isoA2": "PR",
    "isoA3": "PRI",
    "name": "Porto Rico",
    "englishName": "Puerto Rico",
    "continent": "Amériques",
    "dbAliases": [
      "Puerto Rico"
    ]
  },
  {
    "code": "PE",
    "isoA2": "PE",
    "isoA3": "PER",
    "name": "Pérou",
    "englishName": "Peru",
    "continent": "Amériques",
    "dbAliases": [
      "Peru"
    ]
  },
  {
    "code": "DO",
    "isoA2": "DO",
    "isoA3": "DOM",
    "name": "République dominicaine",
    "englishName": "Dominican Republic",
    "continent": "Amériques",
    "dbAliases": [
      "Dominican Republic"
    ]
  },
  {
    "code": "LC",
    "isoA2": "LC",
    "isoA3": "LCA",
    "name": "Sainte-Lucie",
    "englishName": "Saint Lucia",
    "continent": "Amériques",
    "dbAliases": [
      "Saint Lucia"
    ]
  },
  {
    "code": "SV",
    "isoA2": "SV",
    "isoA3": "SLV",
    "name": "Salvador",
    "englishName": "El Salvador",
    "continent": "Amériques",
    "dbAliases": [
      "El Salvador"
    ]
  },
  {
    "code": "TT",
    "isoA2": "TT",
    "isoA3": "TTO",
    "name": "Trinité-et-Tobago",
    "englishName": "Trinidad and Tobago",
    "continent": "Amériques",
    "dbAliases": [
      "Trinidad and Tobago"
    ]
  },
  {
    "code": "UY",
    "isoA2": "UY",
    "isoA3": "URY",
    "name": "Uruguay",
    "englishName": "Uruguay",
    "continent": "Amériques",
    "dbAliases": [
      "Uruguay"
    ]
  },
  {
    "code": "VE",
    "isoA2": "VE",
    "isoA3": "VEN",
    "name": "Venezuela",
    "englishName": "Venezuela",
    "continent": "Amériques",
    "dbAliases": [
      "Venezuela"
    ]
  },
  {
    "code": "EC",
    "isoA2": "EC",
    "isoA3": "ECU",
    "name": "Équateur",
    "englishName": "Ecuador",
    "continent": "Amériques",
    "dbAliases": [
      "Ecuador"
    ]
  },
  {
    "code": "US",
    "isoA2": "US",
    "isoA3": "USA",
    "name": "États-Unis",
    "englishName": "United States",
    "continent": "Amériques",
    "dbAliases": [
      "United States",
      "USA"
    ]
  },
  {
    "code": "TC",
    "isoA2": "TC",
    "isoA3": "TCA",
    "name": "Îles Turques-et-Caïques",
    "englishName": "Turks and Caicos Islands",
    "continent": "Amériques",
    "dbAliases": [
      "Turks and Caicos Islands"
    ]
  },
  {
    "code": "BD",
    "isoA2": "BD",
    "isoA3": "BGD",
    "name": "Bangladesh",
    "englishName": "Bangladesh",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Bangladesh"
    ]
  },
  {
    "code": "MM",
    "isoA2": "MM",
    "isoA3": "MMR",
    "name": "Birmanie",
    "englishName": "Myanmar",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Myanmar"
    ]
  },
  {
    "code": "KH",
    "isoA2": "KH",
    "isoA3": "KHM",
    "name": "Cambodge",
    "englishName": "Cambodia",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Cambodia"
    ]
  },
  {
    "code": "CN",
    "isoA2": "CN",
    "isoA3": "CHN",
    "name": "Chine",
    "englishName": "China",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "China"
    ]
  },
  {
    "code": "KR",
    "isoA2": "KR",
    "isoA3": "KOR",
    "name": "Corée du Sud",
    "englishName": "South Korea",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "South Korea"
    ]
  },
  {
    "code": "HK",
    "isoA2": "HK",
    "isoA3": "HKG",
    "name": "Hong Kong",
    "englishName": "Hong Kong",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Hong Kong"
    ]
  },
  {
    "code": "IN",
    "isoA2": "IN",
    "isoA3": "IND",
    "name": "Inde",
    "englishName": "India",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "India"
    ]
  },
  {
    "code": "ID",
    "isoA2": "ID",
    "isoA3": "IDN",
    "name": "Indonésie",
    "englishName": "Indonesia",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Indonesia"
    ]
  },
  {
    "code": "JP",
    "isoA2": "JP",
    "isoA3": "JPN",
    "name": "Japon",
    "englishName": "Japan",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Japan"
    ]
  },
  {
    "code": "KZ",
    "isoA2": "KZ",
    "isoA3": "KAZ",
    "name": "Kazakhstan",
    "englishName": "Kazakhstan",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Kazakhstan"
    ]
  },
  {
    "code": "MO",
    "isoA2": "MO",
    "isoA3": "MAC",
    "name": "Macao",
    "englishName": "Macau",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Macau",
      "Macau "
    ]
  },
  {
    "code": "MY",
    "isoA2": "MY",
    "isoA3": "MYS",
    "name": "Malaisie",
    "englishName": "Malaysia",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Malaysia"
    ]
  },
  {
    "code": "MV",
    "isoA2": "MV",
    "isoA3": "MDV",
    "name": "Maldives",
    "englishName": "Maldives",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Maldives"
    ]
  },
  {
    "code": "NP",
    "isoA2": "NP",
    "isoA3": "NPL",
    "name": "Népal",
    "englishName": "Nepal",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Nepal"
    ]
  },
  {
    "code": "PH",
    "isoA2": "PH",
    "isoA3": "PHL",
    "name": "Philippines",
    "englishName": "Philippines",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Philippines"
    ]
  },
  {
    "code": "SG",
    "isoA2": "SG",
    "isoA3": "SGP",
    "name": "Singapour",
    "englishName": "Singapore",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Singapore",
      "Singapore "
    ]
  },
  {
    "code": "TW",
    "isoA2": "TW",
    "isoA3": "TWN",
    "name": "Taïwan",
    "englishName": "Taiwan",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Taiwan"
    ]
  },
  {
    "code": "TH",
    "isoA2": "TH",
    "isoA3": "THA",
    "name": "Thaïlande",
    "englishName": "Thailand",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Thailand"
    ]
  },
  {
    "code": "VN",
    "isoA2": "VN",
    "isoA3": "VNM",
    "name": "Viêt Nam",
    "englishName": "Vietnam",
    "continent": "Asie-Pacifique",
    "dbAliases": [
      "Vietnam"
    ]
  },
  {
    "code": "AL",
    "isoA2": "AL",
    "isoA3": "ALB",
    "name": "Albanie",
    "englishName": "Albania",
    "continent": "Europe",
    "dbAliases": [
      "Albania"
    ]
  },
  {
    "code": "DE",
    "isoA2": "DE",
    "isoA3": "DEU",
    "name": "Allemagne",
    "englishName": "Germany",
    "continent": "Europe",
    "dbAliases": [
      "Germany"
    ]
  },
  {
    "code": "AD",
    "isoA2": "AD",
    "isoA3": "AND",
    "name": "Andorre",
    "englishName": "Andorra",
    "continent": "Europe",
    "dbAliases": [
      "Andorra"
    ]
  },
  {
    "code": "AT",
    "isoA2": "AT",
    "isoA3": "AUT",
    "name": "Autriche",
    "englishName": "Austria",
    "continent": "Europe",
    "dbAliases": [
      "Austria"
    ]
  },
  {
    "code": "BE",
    "isoA2": "BE",
    "isoA3": "BEL",
    "name": "Belgique",
    "englishName": "Belgium",
    "continent": "Europe",
    "dbAliases": [
      "Belgium"
    ]
  },
  {
    "code": "BY",
    "isoA2": "BY",
    "isoA3": "BLR",
    "name": "Biélorussie",
    "englishName": "Belarus",
    "continent": "Europe",
    "dbAliases": [
      "Belarus"
    ]
  },
  {
    "code": "BA",
    "isoA2": "BA",
    "isoA3": "BIH",
    "name": "Bosnie-Herzégovine",
    "englishName": "Bosnia and Herzegovina",
    "continent": "Europe",
    "dbAliases": [
      "Bosnia and Herzegovina",
      "Bosnia-Herzegovina"
    ]
  },
  {
    "code": "BG",
    "isoA2": "BG",
    "isoA3": "BGR",
    "name": "Bulgarie",
    "englishName": "Bulgaria",
    "continent": "Europe",
    "dbAliases": [
      "Bulgaria"
    ]
  },
  {
    "code": "CY",
    "isoA2": "CY",
    "isoA3": "CYP",
    "name": "Chypre",
    "englishName": "Cyprus",
    "continent": "Europe",
    "dbAliases": [
      "Cyprus"
    ]
  },
  {
    "code": "HR",
    "isoA2": "HR",
    "isoA3": "HRV",
    "name": "Croatie",
    "englishName": "Croatia",
    "continent": "Europe",
    "dbAliases": [
      "Croatia"
    ]
  },
  {
    "code": "DK",
    "isoA2": "DK",
    "isoA3": "DNK",
    "name": "Danemark",
    "englishName": "Denmark",
    "continent": "Europe",
    "dbAliases": [
      "Denmark"
    ]
  },
  {
    "code": "ES",
    "isoA2": "ES",
    "isoA3": "ESP",
    "name": "Espagne",
    "englishName": "Spain",
    "continent": "Europe",
    "dbAliases": [
      "Spain"
    ]
  },
  {
    "code": "EE",
    "isoA2": "EE",
    "isoA3": "EST",
    "name": "Estonie",
    "englishName": "Estonia",
    "continent": "Europe",
    "dbAliases": [
      "Estonia"
    ]
  },
  {
    "code": "FI",
    "isoA2": "FI",
    "isoA3": "FIN",
    "name": "Finlande",
    "englishName": "Finland",
    "continent": "Europe",
    "dbAliases": [
      "Finland"
    ]
  },
  {
    "code": "GI",
    "isoA2": "GI",
    "isoA3": "GIB",
    "name": "Gibraltar",
    "englishName": "Gibraltar",
    "continent": "Europe",
    "dbAliases": [
      "Gibraltar"
    ]
  },
  {
    "code": "GR",
    "isoA2": "GR",
    "isoA3": "GRC",
    "name": "Grèce",
    "englishName": "Greece",
    "continent": "Europe",
    "dbAliases": [
      "Greece"
    ]
  },
  {
    "code": "HU",
    "isoA2": "HU",
    "isoA3": "HUN",
    "name": "Hongrie",
    "englishName": "Hungary",
    "continent": "Europe",
    "dbAliases": [
      "Hungary"
    ]
  },
  {
    "code": "IE",
    "isoA2": "IE",
    "isoA3": "IRL",
    "name": "Irlande",
    "englishName": "Ireland",
    "continent": "Europe",
    "dbAliases": [
      "Ireland"
    ]
  },
  {
    "code": "IS",
    "isoA2": "IS",
    "isoA3": "ISL",
    "name": "Islande",
    "englishName": "Iceland",
    "continent": "Europe",
    "dbAliases": [
      "Iceland"
    ]
  },
  {
    "code": "IT",
    "isoA2": "IT",
    "isoA3": "ITA",
    "name": "Italie",
    "englishName": "Italy",
    "continent": "Europe",
    "dbAliases": [
      "Italy"
    ]
  },
  {
    "code": "LV",
    "isoA2": "LV",
    "isoA3": "LVA",
    "name": "Lettonie",
    "englishName": "Latvia",
    "continent": "Europe",
    "dbAliases": [
      "Latvia"
    ]
  },
  {
    "code": "LI",
    "isoA2": "LI",
    "isoA3": "LIE",
    "name": "Liechtenstein",
    "englishName": "Liechtenstein",
    "continent": "Europe",
    "dbAliases": [
      "Liechtenstein"
    ]
  },
  {
    "code": "LT",
    "isoA2": "LT",
    "isoA3": "LTU",
    "name": "Lituanie",
    "englishName": "Lithuania",
    "continent": "Europe",
    "dbAliases": [
      "Lithuania"
    ]
  },
  {
    "code": "LU",
    "isoA2": "LU",
    "isoA3": "LUX",
    "name": "Luxembourg",
    "englishName": "Luxembourg",
    "continent": "Europe",
    "dbAliases": [
      "Luxembourg"
    ]
  },
  {
    "code": "MK",
    "isoA2": "MK",
    "isoA3": "MKD",
    "name": "Macédoine du Nord",
    "englishName": "North Macedonia",
    "continent": "Europe",
    "dbAliases": [
      "North Macedonia"
    ]
  },
  {
    "code": "MT",
    "isoA2": "MT",
    "isoA3": "MLT",
    "name": "Malte",
    "englishName": "Malta",
    "continent": "Europe",
    "dbAliases": [
      "Malta"
    ]
  },
  {
    "code": "MD",
    "isoA2": "MD",
    "isoA3": "MDA",
    "name": "Moldavie",
    "englishName": "Moldova",
    "continent": "Europe",
    "dbAliases": [
      "Moldova"
    ]
  },
  {
    "code": "ME",
    "isoA2": "ME",
    "isoA3": "MNE",
    "name": "Monténégro",
    "englishName": "Montenegro",
    "continent": "Europe",
    "dbAliases": [
      "Montenegro"
    ]
  },
  {
    "code": "NO",
    "isoA2": "NO",
    "isoA3": "NOR",
    "name": "Norvège",
    "englishName": "Norway",
    "continent": "Europe",
    "dbAliases": [
      "Norway"
    ]
  },
  {
    "code": "NL",
    "isoA2": "NL",
    "isoA3": "NLD",
    "name": "Pays-Bas",
    "englishName": "Netherlands",
    "continent": "Europe",
    "dbAliases": [
      "Netherlands"
    ]
  },
  {
    "code": "PL",
    "isoA2": "PL",
    "isoA3": "POL",
    "name": "Pologne",
    "englishName": "Poland",
    "continent": "Europe",
    "dbAliases": [
      "Poland"
    ]
  },
  {
    "code": "PT",
    "isoA2": "PT",
    "isoA3": "PRT",
    "name": "Portugal",
    "englishName": "Portugal",
    "continent": "Europe",
    "dbAliases": [
      "Portugal"
    ]
  },
  {
    "code": "RO",
    "isoA2": "RO",
    "isoA3": "ROU",
    "name": "Roumanie",
    "englishName": "Romania",
    "continent": "Europe",
    "dbAliases": [
      "Romania"
    ]
  },
  {
    "code": "GB",
    "isoA2": "GB",
    "isoA3": "GBR",
    "name": "Royaume-Uni",
    "englishName": "United Kingdom",
    "continent": "Europe",
    "dbAliases": [
      "England (UK)",
      "Northern Ireland (UK)",
      "Scotland (UK)",
      "United Kingdom",
      "Wales (UK)"
    ]
  },
  {
    "code": "RU",
    "isoA2": "RU",
    "isoA3": "RUS",
    "name": "Russie",
    "englishName": "Russia",
    "continent": "Europe",
    "dbAliases": [
      "Russia"
    ]
  },
  {
    "code": "CZ",
    "isoA2": "CZ",
    "isoA3": "CZE",
    "name": "République tchèque",
    "englishName": "Czech Republic",
    "continent": "Europe",
    "dbAliases": [
      "Czech Republic"
    ]
  },
  {
    "code": "SM",
    "isoA2": "SM",
    "isoA3": "SMR",
    "name": "Saint-Marin",
    "englishName": "San Marino",
    "continent": "Europe",
    "dbAliases": [
      "San Marino"
    ]
  },
  {
    "code": "RS",
    "isoA2": "RS",
    "isoA3": "SRB",
    "name": "Serbie",
    "englishName": "Serbia",
    "continent": "Europe",
    "dbAliases": [
      "Serbia"
    ]
  },
  {
    "code": "SK",
    "isoA2": "SK",
    "isoA3": "SVK",
    "name": "Slovaquie",
    "englishName": "Slovakia",
    "continent": "Europe",
    "dbAliases": [
      "Slovakia",
      "Slovakia "
    ]
  },
  {
    "code": "SI",
    "isoA2": "SI",
    "isoA3": "SVN",
    "name": "Slovénie",
    "englishName": "Slovenia",
    "continent": "Europe",
    "dbAliases": [
      "Slovenia"
    ]
  },
  {
    "code": "CH",
    "isoA2": "CH",
    "isoA3": "CHE",
    "name": "Suisse",
    "englishName": "Switzerland",
    "continent": "Europe",
    "dbAliases": [
      "Switzerland",
      "Switzerland "
    ]
  },
  {
    "code": "SE",
    "isoA2": "SE",
    "isoA3": "SWE",
    "name": "Suède",
    "englishName": "Sweden",
    "continent": "Europe",
    "dbAliases": [
      "Sweden"
    ]
  },
  {
    "code": "TR",
    "isoA2": "TR",
    "isoA3": "TUR",
    "name": "Turquie",
    "englishName": "Turkey",
    "continent": "Europe",
    "dbAliases": [
      "Turkey"
    ]
  },
  {
    "code": "UA",
    "isoA2": "UA",
    "isoA3": "UKR",
    "name": "Ukraine",
    "englishName": "Ukraine",
    "continent": "Europe",
    "dbAliases": [
      "Ukraine"
    ]
  },
  {
    "code": "SA",
    "isoA2": "SA",
    "isoA3": "SAU",
    "name": "Arabie Saoudite",
    "englishName": "Saudi Arabia",
    "continent": "Moyen-Orient",
    "dbAliases": [
      "Saudi Arabia"
    ]
  },
  {
    "code": "AM",
    "isoA2": "AM",
    "isoA3": "ARM",
    "name": "Arménie",
    "englishName": "Armenia",
    "continent": "Moyen-Orient",
    "dbAliases": [
      "Armenia"
    ]
  },
  {
    "code": "AZ",
    "isoA2": "AZ",
    "isoA3": "AZE",
    "name": "Azerbaïdjan",
    "englishName": "Azerbaijan",
    "continent": "Moyen-Orient",
    "dbAliases": [
      "Azerbaijan"
    ]
  },
  {
    "code": "BH",
    "isoA2": "BH",
    "isoA3": "BHR",
    "name": "Bahreïn",
    "englishName": "Bahrain",
    "continent": "Moyen-Orient",
    "dbAliases": [
      "Bahrain"
    ]
  },
  {
    "code": "GE",
    "isoA2": "GE",
    "isoA3": "GEO",
    "name": "Géorgie",
    "englishName": "Georgia",
    "continent": "Moyen-Orient",
    "dbAliases": [
      "Georgia"
    ]
  },
  {
    "code": "IL",
    "isoA2": "IL",
    "isoA3": "ISR",
    "name": "Israël",
    "englishName": "Israel",
    "continent": "Moyen-Orient",
    "dbAliases": [
      "Israel"
    ]
  },
  {
    "code": "JO",
    "isoA2": "JO",
    "isoA3": "JOR",
    "name": "Jordanie",
    "englishName": "Jordan",
    "continent": "Moyen-Orient",
    "dbAliases": [
      "Jordan",
      "Jordan "
    ]
  },
  {
    "code": "KW",
    "isoA2": "KW",
    "isoA3": "KWT",
    "name": "Koweït",
    "englishName": "Kuwait",
    "continent": "Moyen-Orient",
    "dbAliases": [
      "Kuwait"
    ]
  },
  {
    "code": "LB",
    "isoA2": "LB",
    "isoA3": "LBN",
    "name": "Liban",
    "englishName": "Lebanon",
    "continent": "Moyen-Orient",
    "dbAliases": [
      "Lebanon"
    ]
  },
  {
    "code": "QA",
    "isoA2": "QA",
    "isoA3": "QAT",
    "name": "Qatar",
    "englishName": "Qatar",
    "continent": "Moyen-Orient",
    "dbAliases": [
      "Qatar"
    ]
  },
  {
    "code": "AE",
    "isoA2": "AE",
    "isoA3": "ARE",
    "name": "Émirats arabes unis",
    "englishName": "United Arab Emirates",
    "continent": "Moyen-Orient",
    "dbAliases": [
      "UAE",
      "United Arab Emirates"
    ]
  },
  {
    "code": "AU",
    "isoA2": "AU",
    "isoA3": "AUS",
    "name": "Australie",
    "englishName": "Australia",
    "continent": "Océanie",
    "dbAliases": [
      "Australia"
    ]
  },
  {
    "code": "FJ",
    "isoA2": "FJ",
    "isoA3": "FJI",
    "name": "Fidji",
    "englishName": "Fiji",
    "continent": "Océanie",
    "dbAliases": [
      "Fiji"
    ]
  },
  {
    "code": "GU",
    "isoA2": "GU",
    "isoA3": "GUM",
    "name": "Guam",
    "englishName": "Guam",
    "continent": "Océanie",
    "dbAliases": [
      "Guam"
    ]
  },
  {
    "code": "NC",
    "isoA2": "NC",
    "isoA3": "NCL",
    "name": "Nouvelle-Calédonie",
    "englishName": "New Caledonia",
    "continent": "Océanie",
    "dbAliases": [
      "New Caledonia"
    ]
  },
  {
    "code": "NZ",
    "isoA2": "NZ",
    "isoA3": "NZL",
    "name": "Nouvelle-Zélande",
    "englishName": "New Zealand",
    "continent": "Océanie",
    "dbAliases": [
      "New Zealand"
    ]
  },
  {
    "code": "PF",
    "isoA2": "PF",
    "isoA3": "PYF",
    "name": "Polynésie française",
    "englishName": "French Polynesia",
    "continent": "Océanie",
    "dbAliases": [
      "French Polynesia"
    ]
  },
  {
    "code": "CK",
    "isoA2": "CK",
    "isoA3": "COK",
    "name": "Îles Cook",
    "englishName": "Cook Islands",
    "continent": "Océanie",
    "dbAliases": [
      "Cook Islands"
    ]
  }
];

export const DB_NAME_TO_CODE: Record<string, string> = {
  "albania": "AL",
  "andorra": "AD",
  "angola": "AO",
  "antigua and barbuda": "AG",
  "argentina": "AR",
  "armenia": "AM",
  "aruba": "AW",
  "australia": "AU",
  "austria": "AT",
  "azerbaijan": "AZ",
  "bahamas": "BS",
  "bahrain": "BH",
  "bangladesh": "BD",
  "barbados": "BB",
  "belarus": "BY",
  "belgium": "BE",
  "bermuda": "BM",
  "bolivia": "BO",
  "bonaire": "BQ",
  "bosnia and herzegovina": "BA",
  "bosnia-herzegovina": "BA",
  "botswana": "BW",
  "brazil": "BR",
  "bulgaria": "BG",
  "cambodia": "KH",
  "cameroon": "CM",
  "canada": "CA",
  "cape verde": "CV",
  "chile": "CL",
  "china": "CN",
  "colombia": "CO",
  "cook islands": "CK",
  "costa rica": "CR",
  "cote d`ivoire": "CI",
  "croatia": "HR",
  "cuba": "CU",
  "curacao": "CW",
  "cyprus": "CY",
  "czech republic": "CZ",
  "denmark": "DK",
  "djibouti": "DJ",
  "dominican republic": "DO",
  "ecuador": "EC",
  "egypt": "EG",
  "el salvador": "SV",
  "england (uk)": "GB",
  "estonia": "EE",
  "ethiopia": "ET",
  "fiji": "FJ",
  "finland": "FI",
  "french polynesia": "PF",
  "gabon": "GA",
  "georgia": "GE",
  "germany": "DE",
  "ghana": "GH",
  "gibraltar": "GI",
  "greece": "GR",
  "guam": "GU",
  "guatemala": "GT",
  "haiti": "HT",
  "honduras": "HN",
  "hong kong": "HK",
  "hungary": "HU",
  "iceland": "IS",
  "india": "IN",
  "indonesia": "ID",
  "ireland": "IE",
  "israel": "IL",
  "italy": "IT",
  "jamaica": "JM",
  "japan": "JP",
  "jordan": "JO",
  "kazakhstan": "KZ",
  "kenya": "KE",
  "kuwait": "KW",
  "latvia": "LV",
  "lebanon": "LB",
  "liechtenstein": "LI",
  "lithuania": "LT",
  "luxembourg": "LU",
  "macau": "MO",
  "madagascar": "MG",
  "malaysia": "MY",
  "maldives": "MV",
  "malta": "MT",
  "martinique": "MQ",
  "mauritius": "MU",
  "mexico": "MX",
  "moldova": "MD",
  "montenegro": "ME",
  "morocco": "MA",
  "mozambique": "MZ",
  "myanmar": "MM",
  "namibia": "NA",
  "nepal": "NP",
  "netherlands": "NL",
  "new caledonia": "NC",
  "new zealand": "NZ",
  "nicaragua": "NI",
  "nigeria": "NG",
  "north macedonia": "MK",
  "northern ireland (uk)": "GB",
  "norway": "NO",
  "panama": "PA",
  "paraguay": "PY",
  "peru": "PE",
  "philippines": "PH",
  "poland": "PL",
  "portugal": "PT",
  "puerto rico": "PR",
  "qatar": "QA",
  "romania": "RO",
  "russia": "RU",
  "rwanda": "RW",
  "saint lucia": "LC",
  "san marino": "SM",
  "saudi arabia": "SA",
  "scotland (uk)": "GB",
  "senegal": "SN",
  "serbia": "RS",
  "seychelles": "SC",
  "singapore": "SG",
  "slovakia": "SK",
  "slovenia": "SI",
  "south africa": "ZA",
  "south korea": "KR",
  "spain": "ES",
  "st. helena": "SH",
  "sweden": "SE",
  "switzerland": "CH",
  "taiwan": "TW",
  "tanzania": "TZ",
  "thailand": "TH",
  "trinidad and tobago": "TT",
  "turkey": "TR",
  "turks and caicos islands": "TC",
  "uae": "AE",
  "uganda": "UG",
  "ukraine": "UA",
  "united arab emirates": "AE",
  "united kingdom": "GB",
  "united states": "US",
  "uruguay": "UY",
  "usa": "US",
  "venezuela": "VE",
  "vietnam": "VN",
  "wales (uk)": "GB",
  "zambia": "ZM",
  "zimbabwe": "ZW"
};

export const APP_TO_COUNTRY: Record<string, CountryDef> = Object.fromEntries(
  COUNTRIES.map((country) => [country.code, country])
);

export const ISO3_TO_CODE: Record<string, string> = Object.fromEntries(
  COUNTRIES.map((country) => [country.isoA3, country.code])
);

export const UNMAPPED_COUNTRY_VALUES: string[] = [];
